...
%
% Example:
%   data = importfile('data.dat', 1, 94);
%
%    See also TEXTSCAN.

% Auto-generated by MATLAB on 2015/11/18 11:18:11
...
R = cellfun(@(x) ischar(x) && strcmp(x,'bus'),raw); % Find non-numeric cells
raw(R) = {1.0}; % Replace non-numeric cells

%% Replace specified string with 2.0
R = cellfun(@(x) ischar(x) && strcmp(x,'opel'),raw); % Find non-numeric cells
raw(R) = {2.0}; % Replace non-numeric cells

%% Replace specified string with 3.0
R = cellfun(@(x) ischar(x) && strcmp(x,'saab'),raw); % Find non-numeric cells
raw(R) = {3.0}; % Replace non-numeric cells

%% Replace specified string with 4.0
R = cellfun(@(x) ischar(x) && strcmp(x,'van'),raw); % Find non-numeric cells
